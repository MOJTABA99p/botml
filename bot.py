...
bot = Bot("ovcnpokcpqhlyppeoxfuechzgkupffva")
target = "g0BabeD05dff94ccbfec6a565b4c146b"
...
def hasAds(msg):
	links = ["join","rubika.ir/post","joinc","rubika.ir","@"] # you can develop it
	for i in links:
		if i in msg.lower():
			return True
...
def alert(guid,user,alert_text=""):
	...
	max_alert = 30    # you can change it
						...
						if msg["author_object_guid"] in admins:
							...
							# 	bot.sendMessage(target, "✅ قفل گیف و استیکر غیرفعال شد .", msg["message_id"])
								...
								try:
									...
									bot.sendPhoto(target, "./shot.jpg", [650,370], caption="..", message_id=msg["message_id"])
									os.remove('./shot.jpg')
								except:
